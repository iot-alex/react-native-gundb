;(function(wsp){
	var Gun = require('../gun')
	, ws = require('ws').Server // PL: FIX RN WebSocket
	// , http = require('./http')  // PL: not supported on RN
	, url = require('url');
	Gun.on('opt').event(function(gun, opt){
		...
		gun.wsp.server = gun.wsp.server || function(req, res, next){ // http
			...
			if(msg.url.pathname.replace(gun.wsp.regex,'').slice(0,3).toLowerCase() === '.js'){
				...
				res.end(gun.wsp.js = gun.wsp.js || require('react-native-fs').readFileSync(__dirname + '/../gun.js')); // gun server is caching the gun library for the client
				return true;
			}
		...
		// HTTP and HTTPS not valid for RN
